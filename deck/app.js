(() => {
  ...
  const state = {
    ...
    selected: {},
    admin: { unlocked:false, useOverride:false, overridePct:null, discountEnabled:false, discountType:"percent", discountValue:0 }
  };
  ...
  function render(){
    ...
    if(state.admin?.unlocked && state.admin?.discountEnabled){
      ...
      if(state.admin?.discountType === "fixed") discountAmt = clamp(v, 0, pricedTotal);
      else discountAmt = clamp(pricedTotal * (v/100), 0, pricedTotal);
    }
    const finalTotal = pricedTotal - discountAmt;
    const dRow = $("#discountRow");
    const dVal = $("#discountValueOut");
    if(dRow) dRow.style.display = discountAmt>0 ? "" : "none";
    if(dVal) dVal.textContent = "-" + fmtMoney(discountAmt);
  ...
  (async function boot(){
    ...
    try{
      ...
      console.error(err);
      alert("Deck tool failed to load data.json. Make sure index.html, app.js, styles.css, and data.json are all inside the SAME /deck/ folder in GitHub.");
      return;
    }
    initCompany();
    initTabs();
  ...
  function showAdminUI(){
    ...
    const dEn = document.querySelector("#adminDiscountEnabled");
    const dTy = document.querySelector("#adminDiscountType");
    const dVa = document.querySelector("#adminDiscountValue");
    ...
    if(dTy) dTy.value = state.admin?.discountType || "percent";
    if(dVa) dVa.value = (state.admin?.discountValue ?? "");
  }
  ...
  function wireAdmin(){
    ...
    if(applyBtn){
      applyBtn.addEventListener("click", ()=>{
        ...
        state.admin.discountType = (dTy?.value || "percent");
        state.admin.discountValue = parseNum(dVa?.value);
        save();
        render();
      });
    }
    ...
    if(logo){
      let pressTimer=null;
      const start = (e) => { try{ e?.preventDefault?.(); }catch{} pressTimer=setTimeout(unlockAdmin, 900); };
