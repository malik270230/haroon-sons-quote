...
const inClientName = $("#inClientName");
const inClientAddress = $("#inClientAddress");
  ...
  const printClientBtn = $("#printClientBtn");
  const printInternalBtn = $("#printInternalBtn");
  const printBothBtn = $("#printBothBtn");
...
const printDateStamp = $("#printDateStamp");

  // Client proposal (bucket summary) print fields
  const clientPrintCompanyName = $("#clientPrintCompanyName");
  const clientPrintCompanyPhone = $("#clientPrintCompanyPhone");
  const clientPrintCompanyEmail = $("#clientPrintCompanyEmail");
  const clientPrintGrandTotal = $("#clientPrintGrandTotal");
  const clientPrintClientName = $("#clientPrintClientName");
  const clientPrintClientAddress = $("#clientPrintClientAddress");
  const clientPrintProjectSummary = $("#clientPrintProjectSummary");
  const clientPrintDateStamp = $("#clientPrintDateStamp");
  const bucketBody = $("#bucketBody");
  const bucketGrandTotal = $("#bucketGrandTotal");
  ...
  function inferBucket(label){
    ...
    if(/kitchen|cabinet|counter|wet bar|bar/.test(s)) return "Kitchen / Wet Bar";
    if(/plumb|toilet|vanity|shower|sink|ejector|upflush/.test(s)) return "Plumbing";
    if(/door|hinge|handle|lock|hardware/.test(s)) return "Doors & Hardware";
    return "General Conditions & Supplies";
  }
...
function calcLine(item, d, perItemPct, rawMaterialsSubtotal){
    ...
    return { id:item.id, label:item.label, bucket:(item.bucket||inferBucket(item.label)), unit:item.unit, qty,
      rawMatRate, rawLabRate,
      matRate, labRate,
      rawMatTotal, rawLabTotal, matTotal, labTotal,
...
const tdMatTot=document.createElement("td"); tdMatTot.textContent=fmtMoney(line.matTotal);
const tdLabTot=document.createElement("td"); tdLabTot.textContent=fmtMoney(line.labTotal);
const tdTot=document.createElement("td"); tdTot.textContent=fmtMoney(line.lineTotal);
tr.appendChild(tdItem); tr.appendChild(tdUnit); tr.appendChild(tdQty); tr.appendChild(tdUnitMat); tr.appendChild(tdUnitLab); tr.appendChild(tdMatTot); tr.appendChild(tdLabTot); tr.appendChild(tdTot);      quoteBody.appendChild(tr);
    }
    ...
    if(printClientName) printClientName.textContent = state.clientName || "—";
    if(printClientAddress) printClientAddress.textContent = state.clientAddress || "—";
    if(printDateStamp) printDateStamp.textContent = new Date().toLocaleString();


    // --- Client proposal bucket summary (Page 1) ---
    if(clientPrintCompanyName) clientPrintCompanyName.textContent = DATA.company.name;
    if(clientPrintCompanyPhone) clientPrintCompanyPhone.textContent = DATA.company.phone;
    if(clientPrintCompanyEmail) clientPrintCompanyEmail.textContent = DATA.company.email;
    if(clientPrintGrandTotal) clientPrintGrandTotal.textContent = fmtMoney(grandTotal);
    if(clientPrintClientName) clientPrintClientName.textContent = state.clientName || "—";
    if(clientPrintClientAddress) clientPrintClientAddress.textContent = state.clientAddress || "—";
    if(clientPrintProjectSummary) clientPrintProjectSummary.textContent = `Floor: ${fmtNum(state.floorSF,0)} SF • Ceiling: ${fmtNum(state.ceilFt,0)} ft • Perimeter: ${fmtNum(perimLF,0)} LF`;
    if(clientPrintDateStamp) clientPrintDateStamp.textContent = new Date().toLocaleString();

    // Bucket aggregation (category totals)
    if(bucketBody){
      const sums = {};
      for(const line of lines){
        const b = line.bucket || "General Conditions & Supplies";
        sums[b] = (sums[b] || 0) + line.lineTotal;
      }
      const order = Array.isArray(DATA.bucketOrder) && DATA.bucketOrder.length ? DATA.bucketOrder : Object.keys(sums).sort();
      bucketBody.innerHTML = "";
      for(const b of order){
        const v = sums[b] || 0;
        if(v <= 0.000001) continue;
        const tr = document.createElement("tr");
        const tdL = document.createElement("td"); tdL.className="left"; tdL.textContent = b;
        const tdR = document.createElement("td"); tdR.textContent = fmtMoney(v);
        tr.appendChild(tdL); tr.appendChild(tdR);
        bucketBody.appendChild(tr);
      }
    }
    if(bucketGrandTotal) bucketGrandTotal.textContent = fmtMoney(grandTotal);
  ...
  function initInputs(){
    ...
    if(inClientName) inClientName.value = state.clientName || "";
    if(inClientAddress) inClientAddress.value = state.clientAddress || "";

    [inFloorSF,inCeilFt,inLenFt,inWidFt,inPerimLF].forEach(inp=>inp.addEventListener("input",()=>{ saveState(); recalcAndRender(); }));
    if(inClientName) inClientName.addEventListener("input",()=>{ saveState(); recalcAndRender(); });
    if(inClientAddress) inClientAddress.addEventListener("input",()=>{ saveState(); recalcAndRender(); });
  }

  function initActions(){
    function setPrintMode(mode){ document.body.dataset.print = mode; }
    printClientBtn?.addEventListener("click",()=>{ setPrintMode("client"); recalcAndRender(); window.print(); });
    printInternalBtn?.addEventListener("click",()=>{ setPrintMode("internal"); recalcAndRender(); window.print(); });
    printBothBtn?.addEventListener("click",()=>{ setPrintMode("both"); recalcAndRender(); window.print(); });
    window.addEventListener("afterprint",()=>setPrintMode("both"));
