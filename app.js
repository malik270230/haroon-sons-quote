...
(() => {
  ...
  function calcLine(item, d, perItemPct, rawMaterialsSubtotal){
    ...
    return { id:item.id, label:item.label, unit:item.unit, qty, rawMatTotal, rawLabTotal, matTotal, labTotal, lineTotal: matTotal+labTotal };
  }
  ...
  function recalcAndRender(){
    ...
    // Customer info (prints on PDF)
    if(inClientName) state.clientName = String(inClientName.value||"").trim();
    if(inClientAddress) state.clientAddress = String(inClientAddress.value||"").trim();
    ...
    for(const line of lines){
      ...
      tr.appendChild(tdItem); tr.appendChild(tdUnit); tr.appendChild(tdQty); tr.appendChild(tdMat); tr.appendChild(tdLab); tr.appendChild(tdTot);
    ...
    if(printProjectSummary){
      const bits = [];
      if(state.clientName) bits.push(`Client: ${state.clientName}`);
      if(state.clientAddress) bits.push(`Address: ${state.clientAddress}`);
      bits.push(`Project: ${fmtNum(state.floorSF,0)} SF • Ceiling: ${fmtNum(state.ceilFt,0)} ft • Perimeter: ${fmtNum(perimLF,0)} LF`);
      printProjectSummary.textContent = bits.join("  |  ");
    }
  ...
  function initInputs(){
    ...
    [inClientName,inClientAddress].filter(Boolean).forEach(inp=>inp.addEventListener("input",()=>{ saveState(); recalcAndRender(); }));
  }
